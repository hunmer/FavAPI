      ...
      >
          ...
          {error ? (
            <div className="p-8 text-center space-y-3">
              ...
              <button
                type="button"
                onClick={load}
                className="px-4 py-2 bg-slate-900 text-white text-xs font-bold rounded-xl inline-flex items-center gap-1.5"
              >
            ...
            <>
              <div className="bg-amber-50/70 text-amber-900 text-xs p-3 rounded-2xl border border-amber-200/60 flex items-center justify-between">
                <span className="flex items-center gap-1.5">
                  <Shield className="w-4 h-4 text-amber-600" />
                  Cookies 仅在私有化部署实例内部调用，已与本地 Profile 关联同步。
                </span>
                <span className="font-bold text-[11px] bg-amber-100 px-2 py-0.5 rounded-full">
                  共 {cookies.length} 条项
                </span>
              </div>
                    ...
                    <tbody className="divide-y divide-slate-100 font-mono">
                      {cookies.map((c, i) => (
                        <tr key={i} className="hover:bg-slate-50/80 transition-colors">
                          ...
                          <td className="py-2.5 px-3 text-slate-600 max-w-[150px] truncate" title={c.value}>
                            {c.value.length > 16 ? `${c.value.slice(0, 16)}...` : c.value || '—'}
                          </td>
                          <td className="py-2.5 px-3 text-slate-500">{c.domain}</td>
                          <td className="py-2.5 px-3 text-slate-500 whitespace-nowrap">{c.expires}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
        ...
        <div className="p-4 sm:p-5 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
          <button
            ...
            disabled={!cookies}
            className="text-xs font-bold text-slate-700 hover:text-slate-900 px-4 py-2 rounded-xl bg-white border border-slate-200 hover:bg-slate-100 transition-colors inline-flex items-center gap-1.5 disabled:opacity-50"
          >
